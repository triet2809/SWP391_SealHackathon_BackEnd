...
public class RoundController {
    ...
    public ResponseEntity<Page<RoundResponse>> listByTrack(
            @RequestParam(required = false) UUID trackId, Pageable pageable) {
        return ResponseEntity.ok(roundService.listByTrack(trackId, pageable));
    }
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('COORDINATOR')")
    @Operation(summary = "Delete round (event must still be draft)")
