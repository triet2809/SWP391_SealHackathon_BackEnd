...
public class ScoreService {
    private final ScoreRepository scoreRepository; private final SubmissionRepository submissionRepository; private final RoundCriterionRepository criterionRepository; private final UserRepository userRepository; private final RoundJudgeRepository roundJudgeRepository;
    @Transactional(readOnly=true) public Page<ScoreResponse> list(UUID submissionId, UUID judgeId, Pageable pageable){ if(submissionId!=null)return scoreRepository.findBySubmissionId(submissionId,pageable).map(ScoreMapper::toResponse); if(judgeId!=null)return scoreRepository.findByJudgeId(judgeId,pageable).map(ScoreMapper::toResponse); return scoreRepository.findAll(pageable).map(ScoreMapper::toResponse); }
    @Transactional(readOnly=true) public ScoreResponse get(UUID id){return ScoreMapper.toResponse(findOrThrow(id));}
