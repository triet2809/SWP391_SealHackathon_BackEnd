...
public final class RoundCriterionMapper {
    ...
    public static RoundCriterionResponse toResponse(RoundCriterion c) {
        return RoundCriterionResponse.builder().id(c.getId()).roundId(c.getRound().getId()).templateId(c.getTemplateId()).name(c.getName()).weight(c.getWeight()).description(c.getDescription()).createdAt(c.getCreatedAt()).updatedAt(c.getUpdatedAt()).build();
    }
