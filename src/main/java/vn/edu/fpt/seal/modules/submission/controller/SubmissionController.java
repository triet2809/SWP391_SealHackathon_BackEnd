...
public class SubmissionController {
    ...
    public ResponseEntity<Page<SubmissionResponse>> list(@RequestParam(required = false) UUID roundId, @RequestParam(required = false) UUID teamId, @RequestParam(required = false) UUID trackId, Pageable pageable) { return ResponseEntity.ok(submissionService.list(roundId, teamId, trackId, pageable)); }
    @GetMapping("/{id}") @PreAuthorize("isAuthenticated()") @Operation(summary = "Get submission")
    public ResponseEntity<SubmissionResponse> get(@PathVariable UUID id) { return ResponseEntity.ok(submissionService.get(id)); }
