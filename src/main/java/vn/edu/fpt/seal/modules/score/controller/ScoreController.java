...
public class ScoreController {
    ...
    @GetMapping @PreAuthorize("isAuthenticated()") public ResponseEntity<Page<ScoreResponse>> list(@RequestParam(required=false) UUID submissionId,@RequestParam(required=false) UUID judgeId, Pageable pageable){return ResponseEntity.ok(service.list(submissionId,judgeId,pageable));}
    @GetMapping("/{id}") @PreAuthorize("isAuthenticated()") public ResponseEntity<ScoreResponse> get(@PathVariable UUID id){return ResponseEntity.ok(service.get(id));}
